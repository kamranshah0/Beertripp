...
if ( ! class_exists( 'acf_field_frontend_blocks' ) ) :

	class acf_field_frontend_blocks extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			$this->label    = __( 'Blocks', FEA_NS );
			$this->category = __( 'Form', FEA_NS );
			$this->defaults = array(
				'layouts'      => array(),
				'min'          => '',
				'max'          => '',
				'button_label' => __( 'Add Row', FEA_NS ),
			);

			// ajax
			$this->add_action( 'wp_ajax_acf/fields/frontend_blocks/layout_title', array( $this, 'ajax_layout_title' ) );
			$this->add_action( 'wp_ajax_nopriv_acf/fields/frontend_blocks/layout_title', array( $this, 'ajax_layout_title' ) );
		...
		function input_admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					...
					// identifiers
					'layout'  => __( 'layout', FEA_NS ),
					'layouts' => __( 'layouts', FEA_NS ),
					...
					// field settings
					'Blocks requires at least 1 layout' => __( 'Blocks requires at least 1 layout', FEA_NS ),
				)
			);
		}
		...
		*/
		...
		function render_field( $field ) {
			...
			$default_labels = array(
				'remove' 	=> __( 'Remove layout', FEA_NS ),
				'add' 		=> __( 'Add layout', FEA_NS ),
				'duplicate'	=> __( 'Remove layout', FEA_NS ),
				'collapse' 	=> __( 'Click to Toggle', FEA_NS ),
				'button'	=> __( 'Add layout', FEA_NS ),
				'no_value'  => __( 'Click the button below to add your first block layout', FEA_NS ),
			);

			if( isset( $field['layout_labels'] ) ){
				$field['layout_labels'] = wp_parse_args( $field['layout_labels'], $default_labels );
			}else{
				$field['layout_labels'] = $default_labels;
			}
			...
			// no value message
			$no_value_message = $field['layout_labels']['no_value'];
...
<div <?php acf_esc_attr_e( $div ); ?>>
	...
	<div class="values">
			...
			if ( ! empty( $field['value'] ) ) :
				...
				foreach ( $field['value'] as $i => $value ) :
					...
					// validate
					if ( empty( $layouts[ $value['fea_block_structure'] ] ) ) {
						continue;
					}

					// render
					$this->render_layout( $field, $layouts[ $value['fea_block_structure'] ], $i, $value );
	...
	<div class="acf-actions">
		<a class="acf-button button button-primary" href="#" data-name="add-layout"><?php echo acf_esc_html( $field['layout_labels']['button'] ); ?></a>
	</div>
...
<div <?php echo acf_esc_attr( $div ); ?>>
	...
	<div class="acf-frontend-blocks-layout-handle" title="<?php _e( 'Drag to reorder', FEA_NS ); ?>" data-name="collapse-layout"><?php echo acf_esc_html( $title ); ?></div>
	
	<div class="acf-frontend-blocks-layout-controls">
		<a class="acf-icon -plus small light acf-js-tooltip" href="#" data-name="add-layout" title="<?php echo $field['layout_labels']['add']; ?>"></a>
		<a class="acf-icon -duplicate small light acf-js-tooltip" href="#" data-name="duplicate-layout" title="<?php echo $field['layout_labels']['duplicate'] ?>"></a>
		<a class="acf-icon -minus small light acf-js-tooltip" href="#" data-name="remove-layout" title="<?php echo $field['layout_labels']['remove'] ?>"></a>
		<a class="acf-icon -collapse small -clear acf-js-tooltip" href="#" data-name="collapse-layout" title="<?php echo $field['layout_labels']['collapse']; ?>"></a>
	</div>
	...
	<?php endif; ?>
		...
		*/
		...
		function update_row( $row, $i, $field, $post_id ) {
			...
			// get layout
			$layout = $this->get_layout( $row['fea_block_structure'], $field );
		...
		*/
		...
		function get_layout_title( $field, $layout, $i, $value ) {
			...
			// filters
			$title = apply_filters( 'acf/fields/blocks/layout_title', $title, $field, $layout, $i );
			$title = apply_filters( 'acf/fields/blocks/layout_title/name=' . $field['_name'], $title, $field, $layout, $i );
			$title = apply_filters( 'acf/fields/blocks/layout_title/key=' . $field['key'], $title, $field, $layout, $i );
			...
			$title = '<span class="acf-frontend-blocks-layout-order">' . $order . '</span> ' . acf_esc_html( $title );
		...
		*/
		...
		public function validate_rest_value( $valid, $value, $field ) {
			...
			if ( ! is_array( $value ) && is_null( $value ) ) {
				$error = sprintf( __( '%s must be of type array or null.', FEA_NS ), $param );
				return new WP_Error( 'rest_invalid_param', $error, $param );
			}

			$layouts_to_update = array_count_values( array_column( $value, 'fea_block_structure' ) );
		...
		public function format_value_for_rest( $value, $post_id, array $field ) {
			...
			foreach ( $value as &$layout ) {
				$name = $layout['fea_block_structure'];
			...
			foreach ( $field['layouts'] as $layout ) {
				...
				<div class="acf-field acf-field-setting-fc_layout" data-name="fc_layout" data-setting="flexible_content" data-id="<?php echo esc_attr( $layout['key'] ); ?>">
					...
					<div class="acf-input">
						...
						acf_hidden_input(
							array(
								'data-id'   => acf_idify( $layout_prefix . '[key]' ),
	...
	// initialize
	acf_register_field_type( 'acf_field_frontend_blocks' );
